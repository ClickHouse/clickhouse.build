# Expense Tracker (Drizzle ORM Version)

A full-stack expense tracking application built with Next.js, TypeScript, Tailwind CSS, PostgreSQL, and **Drizzle ORM**. Features a modern UI with toast notifications, analytics dashboard, and support for millions of expense records.
...
- ⚡ **High Performance** - Handles millions of records with optimized Drizzle queries
- 🎨 **Theme Support** - Basic dark/light theme detection (body styling only)
- 🗃️ **Type-Safe Database** - Full type safety with Drizzle ORM and TypeScript
...
This will:
- Start PostgreSQL 17 on port 5432 with container name `pg-expense-drizzle-db`
- Create a database named `expense_db`
- Use default credentials (postgres/postgres)
- Initialize the expenses table via `init.sql`

### 4. Set Up Drizzle ORM (Optional)

The application is ready to use with the existing database table created by `init.sql`. The Drizzle schema matches the existing table structure.

For future schema changes, you can use Drizzle Kit commands:

```bash
# Generate migration files from schema changes
npm run db:generate

# Apply migrations to database (for new changes)
npm run db:migrate
...
**Note**: Since the database table already exists and matches the Drizzle schema, `db:generate` will show "No schema changes, nothing to migrate" and `db:migrate` may fail due to the existing table. This is expected behavior.

### 5. Configure Environment Variables

The app uses `.env.local` for database configuration (create this file if it doesn't exist):
...
- **Zero Runtime Overhead**: Generates optimal SQL queries at compile time
- **Schema Management**: Declarative schema definition in TypeScript
- **Migration System**: Version-controlled database schema changes
- **Developer Experience**: Auto-completion, compile-time error checking
- **Studio GUI**: Visual database browser and query builder

### Database Performance

- Uses Drizzle ORM with connection pooling via `node-postgres`
- Type-safe queries with automatic SQL generation
- Optimized aggregation queries for analytics across millions of records
