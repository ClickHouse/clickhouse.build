...
def run_single_eval(test_case: Dict, model: str = DEFAULT_MODEL) -> Dict[str, Any]:
    ...
    try:
        ...
        result = qa_approve(file_path, code, purpose, model=model)
...
def main(model: str = DEFAULT_MODEL):
    ...
    for test_case in ground_truth["test_cases"]:
        result = run_single_eval(test_case, model=model)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run QA code migrator evaluation")
    parser.add_argument(
        "--model",
        type=str,
