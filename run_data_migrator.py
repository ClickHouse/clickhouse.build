...
from src.tools.data_migrator import ReplicationMode, data_migrator
...
@click.option(
    "--database", "-d", prompt="Database name", help="Source PostgreSQL database name"
)
@click.option(
    "--tables", "-t", prompt="Table names (comma-separated)", help="Tables to migrate"
)
@click.option(
    "--mode",
    "-m",
...
def show_interactive_menu():
    ...
    while True:
        ...
        if choice is None or choice == "Exit":
            ...
            data_choices.extend(
                ["🤖 Read discovery and provide output", "🧙 Human-in-the-loop only"]
            )

            data_approach = questionary.select(
                "How would you like to proceed?", choices=data_choices
            ).ask()
            ...
            if data_approach == "📄 Use migration data plan":
                ...
                click.secho(
                    "\n🔄 Running data migration from code discovery",
                    fg="green",
                    bold=True,
                )
                click.secho("=" * 60, fg="green")
                ...
                try:
                    ...
                    if "assumptions" in result_data and "config" in result_data:
                        ...
                        # Parse the config
                        config_data = (
                            json.loads(result_data["config"])
                            if isinstance(result_data["config"], str)
                            else result_data["config"]
                        )
                ...
                click.echo()
                click.secho(
                    "Add schemas and tables (leave schema empty to finish)", fg="cyan"
                )
                ...
                while True:
                    ...
                    schema = questionary.text(
                        "Schema name (or press Enter to finish):",
                    ...
                    tables = questionary.text(
                        f"Table names for schema '{schema}' (comma-separated):"
                    ).ask()
                    if not tables or tables.strip() == "":
                        click.secho(
                            "No tables provided for this schema, skipping...",
                            fg="yellow",
                        )
                        continue
                    ...
                    click.secho(
                        f"✓ Added {len(table_list)} table(s) from schema '{schema}'",
                        fg="green",
                    )
